...
<template>
  <div class="mx-auto max-w-4xl px-4 py-8 space-y-6">
    <h1 class="text-h2 font-bold text-text-primary">Mes gains</h1>

    <!-- 4 colonnes seulement a partir de md : a 640px (sm) la carte ne fait que 106px de contenu,
         trop etroit pour un montant en euros formate fr-FR (les separateurs U+202F / U+00A0 sont
         insecables, la valeur ne peut donc pas passer a la ligne). -->
    <div class="grid grid-cols-2 gap-4 md:grid-cols-4">
      <Card v-for="c in cards" :key="c.label">
        <CardContent class="p-4">
          <div class="flex items-center gap-2 text-text-muted text-body-sm"><Icon :name="c.icon" class="size-4" />{{ c.label }}</div>
          <p class="mt-1 text-h4 font-bold text-text-primary md:text-h3">{{ eur(c.bucket?.total ?? 0) }}</p>
          <p class="text-caption text-text-muted">{{ c.bucket?.deliveries ?? 0 }} livr. · {{ c.bucket?.returns ?? 0 }} retour(s)</p>
        </CardContent>
      </Card>
    </div>
        ...
        <table class="w-full text-body-sm">
          ...
          <tbody>
            <tr v-for="h in rider.history" :key="h.id" class="border-b border-border/50 last:border-0">
              <td class="px-4 py-2"><span class="rounded-full px-2 py-0.5 text-caption" :class="h.kind === 'return' ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'">{{ h.kind === 'return' ? 'Retour' : 'Livraison' }}</span></td>
    ...
    <EmptyState
      ...
      description="Vos gains apparaîtront ici après votre première livraison."
    >
      <template #icon>
        <Icon name="ph:currency-eur" class="size-10 text-primary-500" />
      </template>
      <template #actions>
